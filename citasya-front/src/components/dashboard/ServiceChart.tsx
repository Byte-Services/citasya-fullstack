...
export const ServicesChart: React.FC<ServicesChartProps> = ({ startDate, endDate }) => {
    ...
    return (
        <div className="rounded-lg shadow-md w-full" style={{ fontFamily: 'Poppins, sans-serif' }}>
            <h2 className="p-6 text-md font-medium text-left mb-4 bg-neutral-100 text-neutral-700 rounded-t-lg">
                Servicios más agendados
            </h2>
            <div className="flex flex-col items-center p-6">
                <div className="relative h-44 w-44 flex items-center justify-center max-sm:h-36 max-sm:w-36">
                ...
                <div className="flex flex-wrap justify-center gap-4 mt-6 max-sm:gap-2">
                    {chartData.map((d) => (
                        <div key={d.label} className="flex items-center gap-2">
                            <div className="h-3 w-3 rounded-full" style={{ backgroundColor: d.color }}></div>
                            <span className="text-sm text-neutral-700 max-sm:text-xs text-center">{d.label}</span>
                        </div>
                    ))}
                </div>
                <div className="flex flex-wrap justify-center gap-6 mt-8 w-full max-sm:gap-4">
                    {chartData.map((d) => (
                        <div key={d.label} className="flex flex-col items-center max-sm:w-[45%] w-[30%] text-center">
                            <span className="text-2xl font-bold max-sm:text-lg" style={{ color: d.color }}>
                                {total > 0 ? ((d.value / total) * 100).toFixed(0) : 0}%
                            </span>
                            <span className="text-sm text-neutral-600 mt-1 max-sm:text-xs">{d.label}</span>
                        </div>
                    ))}
                </div>
            </div>
